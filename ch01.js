registerChapter({
  id: 1,
  title: "Introduction",
  sections: "1.1–1.7 (full chapter)",

  guide: [
    {
      title: "How to use this chapter",
      body: `
Chapter 1 contains essentially no computation. It is vocabulary, one qualitative case
study, and a roadmap of the rest of the book.

It matters for two reasons: it fixes terminology that later chapters use with
precision, and it plants the qualitative intuition (gain versus overshoot versus
steady-state error) that the remaining twelve chapters make quantitative.

> If you retain one thing: raising loop gain speeds up the response, increases
> overshoot, and reduces steady-state error. That trade-off, and the fact that a
> dynamic compensator breaks it, is what Chapters 8–11 formalize.

Because your exam is closed-calculator and application-focused, the value of this chapter
is almost entirely **vocabulary precision**. Exam problems will use *actuating signal*,
*error*, *natural response* and *transient response* as technical terms with exact
meanings, and will punish loose usage. The 25 problems below drill exactly those
distinctions.
`
    },
    {
...
actuating signal equals the true input-minus-output difference. Nise is deliberate about
this and so are exam writers.
...
`
  ...
  formulas: [
    { latex: "\\text{Total response} = \\text{Natural} + \\text{Forced}",
      note: "Natural = homogeneous solution, form depends only on the system. Forced = particular solution, form depends on the input." },
    { latex: "a_n\\frac{d^{n}c}{dt^{n}}+\\cdots+a_0c(t)=b_m\\frac{d^{m}r}{dt^{m}}+\\cdots+b_0r(t)",
      note: "General LTI differential equation. Output c(t), input r(t). Becomes the transfer function in Chapter 2." },
    { latex: "\\delta(t),\\quad u(t),\\quad t\\,u(t),\\quad \\tfrac{1}{2}t^{2}u(t),\\quad \\sin\\omega t",
      note: "The five standard test inputs: impulse, step, ramp, parabola, sinusoid." },
    { latex: "\\text{Stability: natural response} \\to 0 \\text{ or bounded oscillation}",
      note: "If the natural response grows without bound, the system is unstable and the other two objectives are moot." }
  ],
...
A common wrong answer is "a system that uses feedback to control something." That
...
`
    },
    {
      ...
      solution: `
This is the single most commonly confused point in Chapter 1, and Nise puts it in a
footnote where most people skip it.
...
**A notational detail worth noticing.** Newton's second law is normally written
...
**Why it is worth memorizing now.** This system reappears as the case study in Chapters
...
`
    },
    {
      ...
      solution: `
**Human factors.** The elevator is the standing example. If the response is too slow the
...
**A third reason worth knowing.** Transient response is often the direct determinant of
...
`
    },
    {
      ...
      difficulty: "challenge",
      topic: "Design objectives",
      prompt: "Beyond the three main objectives, name two other considerations the text says must be taken into account early in a design, and explain why \"early\" matters for each.",
...
**Worth noticing.** This system is closed loop but its controller is a simple on/off
